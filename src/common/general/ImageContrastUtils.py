...
class ImageContrast(object):
    ...
    def same_as(self, mode_image, now_image, percent=10,err_message="图片不一样"):
        # 对比图片，percent值设为0，则100%相似时返回True，设置的值越大，相差越大
        import math
        import operator

        image1 = Image.open(mode_image)
        image2 = Image.open(now_image)

        histogram1 = image1.histogram()
        histogram2 = image2.histogram()

        differ = math.sqrt(reduce(operator.add, list(map(lambda a, b: (a - b) ** 2, histogram1, histogram2))) / len(histogram1))
        U.Logging.error("不同比： "+str(differ))
        if differ <= percent:
            U.Logging.error("if： " + str(differ))
            raise Exception(err_message)
            return True
        else:
            U.Logging.error("else不同比： " + str(differ))
            return False
